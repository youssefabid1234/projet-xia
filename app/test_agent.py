...
class AgentTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.exercice = {"id": "a", "chapitre": "Analyse", "difficulte": 1,
    ...
    async def test_selection_niveau_et_historique(self):
        self.client.responses.create.side_effect = [
            sortie(appels=[appel("proposer_exercice", chapitre="Analyse")]), sortie("À vous de chercher !"),
            sortie(appels=[appel("consulter_niveau", chapitre="Analyse")]), sortie("Votre niveau est 1,5."),
        ]
    ...
    async def test_reconstruction_privee_avant_proposition_et_correction_sans_reverification(self):
        ...
        self.client.responses.create.side_effect = [
            sortie(appels=[appel("proposer_exercice", chapitre="Analyse")]), sortie(propre)]
        ...
        with patch.dict(os.environ, {"PIPELEX_API_KEY": "test"}), patch("app.agent.PipelexAPIClient") as classe:
            ...
            await self.agent.corriger_derniere_reponse()
    ...
    async def test_ambiguite_ecartee_silencieusement_et_selection_suivante(self):
        ...
        self.client.responses.create.side_effect = [
            sortie(appels=[appel("proposer_exercice", chapitre="Analyse")]), sortie("Calculer $2+2$.")]
        await self.agent.repondre("Un exercice", self.client)
        ...
        outputs = [json.loads(item["output"]) for item in self.agent.historique
                   if item.get("type") == "function_call_output"]
        self.assertEqual(outputs, [{"id": "b", "chapitre": "Analyse", "difficulte": 3,
                                    "enonce": "Calculer $2+2$."}])
        self.assertFalse(self.chemin.exists())
        await self.agent.proposer_exercice("Analyse", self.client)
        self.assertEqual(self.verifier.await_count, 2)
    ...
    async def test_tous_ambigus_ne_sont_pas_marques_vus_et_ne_bouclent_pas(self):
        ...
        for _ in range(2):
            resultat = await self.agent.proposer_exercice("Analyse", self.client)
    ...
    async def test_demande_aide_ne_declenche_pas_pipelex(self):
        await self.agent.proposer_exercice("Analyse")
    ...
    async def test_tentative_courte_utilise_methode_et_persiste_une_fois(self):
        await self.agent.proposer_exercice("Analyse")
        verdict = {"verdict": "correcte", "type_erreur": "aucune", "explication": "La somme vaut 2."}
        with patch.dict(os.environ, {"PIPELEX_API_KEY": "test"}), patch("app.agent.PipelexAPIClient") as classe:
            pipelex = AsyncMock()
            classe.return_value.__aenter__.return_value = pipelex
            pipelex.start_and_wait.return_value.main_stuff = verdict
            self.client.responses.create.side_effect = [
                sortie(appels=[appel("evaluer_reponse", enonce=self.exercice["enonce"], reponse="2")]),
                sortie("Cliquez sur Corriger ma réponse."),
            ]
            await self.agent.repondre("2", self.client)
            pipelex.start_and_wait.assert_not_awaited()
            await self.agent.corriger_derniere_reponse()
            await self.agent.evaluer_reponse(self.exercice["enonce"], "2")
            pipelex.start_and_wait.assert_awaited_once()
            self.assertEqual(pipelex.start_and_wait.call_args.kwargs["pipe_code"],
                             "evaluation_maths_prepa.evaluer_reponse")
            self.assertEqual(pipelex.start_and_wait.call_args.kwargs["inputs"]["reponse_eleve"], "2")
            self.assertEqual(pipelex.start_and_wait.call_args.kwargs["inputs"]["corrige"], "SECRET")
        profil = Profil.charger(self.chemin)
        self.assertAlmostEqual(profil.niveau("Analyse"), 1.8)
        self.assertEqual(len(profil.historique), 1)
        self.assertIn("La somme vaut 2.", self.agent.messages[-1]["content"])
        self.assertIn("1,8/5", self.agent.messages[-1]["content"])
        self.assertIsNone(self.agent.derniere_reponse)
        with self.assertRaises(ValueError):
            await self.agent.corriger_derniere_reponse()

    def test_selection_elargit_jusqua_epuisement_du_chapitre(self):
        profil = Profil("test")
        profil.niveaux["Analyse"] = 3
        ...
        difficultes = []
        for _ in range(5):
            exercice = choisir_exercice(profil, "Analyse", exercices)
            difficultes.append(exercice["difficulte"])
            profil.exercices_vus.append(exercice["id"])
        self.assertEqual(difficultes, [3, 4, 2, 5, 1])
        self.assertIsNone(choisir_exercice(profil, "Analyse", exercices))

    async def test_nouvel_exercice_invalide_reponse_precedente(self):
        self.agent.exercices.append(dict(self.exercice, id="b", difficulte=5))
        await self.agent.proposer_exercice("Analyse")
        self.client.responses.create.return_value = sortie("Cliquez pour corriger.")
        await self.agent.repondre("2", self.client)
        self.assertEqual(self.agent.derniere_reponse, "2")
        self.assertEqual((await self.agent.proposer_exercice("Analyse"))["id"], "b")
        with self.assertRaises(ValueError):
            await self.agent.corriger_derniere_reponse()
        self.assertIn("information", await self.agent.proposer_exercice("Analyse"))
    ...
    async def test_vrai_sdk_avec_transport_simule(self):
        ...
        def transport(requete):
            ...
            if len(requetes) == 1:
                output = [{"type": "function_call", "id": "fc1", "call_id": "c1",
                           "name": "consulter_niveau", "arguments": '{"chapitre":"Analyse"}'}]
            else:
        ...
        async with AsyncOpenAI(api_key="test", http_client=httpx.AsyncClient(
            ...
            texte = await self.agent.repondre("Quel est mon niveau en Analyse ?", client)
